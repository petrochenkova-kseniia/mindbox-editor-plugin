...
def main() -> None:
    ...
    if token is None:
        die(
            ...
            "Открой Терминал и один раз выполни:\n"
            f"  yc init --federation-id={FEDERATION_ID}\n"
            "На «Press enter to continue» — Enter. На выбор профиля — 1.\n"
            "Откроется браузер с логином Mindbox SSO, пройди как обычно.\n"
            "Если в браузере выскочит «Создайте облако» — закрой вкладку.\n"
            "Если в Терминале «no cloud available» — это не ошибка, игнорируй.\n"
            "После этого повтори запрос — токен возьмётся автоматически."
        )
    ...
    for result in results:
        ...
        if err and err.get("http_code") == 403:
            die(
                ...
                "она передаст в инфру, чтобы тебя добавили в группу доступа.\n"
                "Как только подтвердит, что готово — повтори запрос.",
                code=3,
            )
